...
<head>
    ...
    <link rel="icon" type="image/svg+xml" href="{{ asset('infinite-finances-black.svg') }}">
    <link rel="shortcut icon" href="{{ asset('infinite-finances-black.svg') }}">
...
<body class="min-h-screen font-['Inter'] antialiased {{ $pageType ?? '' }}">
    @if ($header === 'header-extended')
        <x-header-home :hero-tag="$heroTag" :hero-title="$heroTitle" :hero-description="$heroDescription" />
    @elseif ($header === 'header-regular')
        <x-header-regular :hero-tag="$heroTag" :hero-title="$heroTitle" :hero-description="$heroDescription" />
    @elseif ($header === 'header-blog-post')
        <x-header-blog-post :hero-tag="$heroTag" :hero-title="$heroTitle" :hero-description="$heroDescription" />
    @else
        <x-header-regular :hero-tag="$heroTag" :hero-title="$heroTitle" :hero-description="$heroDescription" />
    @endif
    ...
    {{-- <header class="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-8 sm:py-5">
            <div class="flex items-center gap-4">
                <div>
                    <a href="/" class="inline-block">
                        <img src="{{ asset('infinite-finances-black.svg') }}" alt="Infinite Finances"
                            class="h-8 w-auto max-w-[50vw] object-contain sm:max-w-none">
                    </a>
            ...
            <div class="hidden items-center gap-3 text-sm text-slate-700 md:flex">
                <a href="/charts"
                    class="rounded-lg px-3 py-2 font-semibold text-blue-600 transition hover:bg-blue-50">Historical
                    Charts</a>
                <a href="/blog" class="rounded-lg px-3 py-2 font-medium transition hover:bg-slate-100">Blog</a>
                <a href="/api-docs" class="rounded-lg px-3 py-2 font-medium transition hover:bg-slate-100">API
                    Docs</a>
        ...
        <div id="mobile-menu" class="hidden border-t border-slate-200 px-4 py-3 md:hidden">
            <div class="flex flex-col gap-2">
                <a href="/charts"
                    class="rounded-lg bg-blue-50 px-3 py-2 text-center font-semibold text-blue-600 transition hover:bg-blue-100">Historical
                    Charts</a>
                <a href="/blog"
                    class="rounded-lg px-3 py-2 text-center font-medium transition hover:bg-slate-100">Blog</a>
                <a href="/faq"
                    class="rounded-lg px-3 py-2 text-center font-medium transition hover:bg-slate-100">FAQ</a>
                <a href="/api-docs"
                    class="rounded-lg px-3 py-2 text-center font-medium transition hover:bg-slate-100">API Docs</a>
    ...
    <footer class="mt-auto border-t border-slate-200">
        <div class="mx-auto max-w-7xl px-8 py-8">
            <div class="mb-6 space-y-4">
                ...
                <details
                    class="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600">
